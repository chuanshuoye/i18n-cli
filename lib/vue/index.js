...
function createScriptSpaces(node) {
  const { content, loc } = node;
  let code = content;
  const startLine = loc.start.line;
  const endLine = loc.end.line;
  for (let loc = startLine; loc < endLine - 1; loc++) {
    code = '\n' + code;
  }
  return code;
}
...
function scan(ast, path) {
  ...
  children && children.forEach((i, index) => {
    ...
    if (i.tag === 'script') {
      ...
      const scriptNode = index > 0 ? createScriptSpaces({ content, loc }) : content;
      babel.babelParse({ code: scriptNode, loc }, scanNodeList);
    }
  })

  // console.log('scanNodeList', scanNodeList)
